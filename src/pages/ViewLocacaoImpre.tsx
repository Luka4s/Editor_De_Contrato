import { Background } from "../components/Background/Background";
import { Download } from "../components/Download/Download";
import { EditTable } from "../components/EditarTable/EditTable";
import { MyModal } from "../components/Modal/Modal";
import styles from "../components/Templates/StylesTeamplates.module.css";
import { ViewInputsLocacaoImpre } from "../components/ViewInputs/LocaçãoImpressoras";
import { VisualizationLocacaoImpre } from "../components/Visualization/VizualizationLocacaoImpre";

export function ViewLocacaoImpressoras() {
  return (
    <div>
      <div>
        <Background />
      </div>

      {/* <div className={styles.buttonSwitchContainer}>
        <button
          className={styles.buttonSwitch}
          onClick={() => {
            setSwitchHiper(!switchHiper);
          }}
        >
          <span>
            {switchHiper ? "Proposta" : "Contrato"}
            <TbSwitchHorizontal />
          </span>
        </button>
      </div> */}
      <div className={styles.wrapper}>
        <div className={styles.menu}>
          <div className={styles.info}>
            <h2>Preencha os campos abaixo:</h2>
          </div>
          <div className={styles.InputsContainer}>
            <ViewInputsLocacaoImpre />
          </div>
          <section className={styles.sectionDownload}>
            <Download />
          </section>
        </div>
        <main>
          <div>
            <VisualizationLocacaoImpre />
            <MyModal />
            <EditTable />
          </div>
        </main>
      </div>
    </div>
  );
}
